...
The agents do not reach the Wayfinder data through the REST API. They go through
the MCP server, which is a separate protected resource with its own scopes.
Register it in Asgardeo so tokens can be issued for it.

- Navigate to Resources > MCP Servers
- Click on + New MCP Server
- Under Basic Details:
    - Identifier: `https://api.wayfinderapp.com`
    - Display Name: `Wayfinder MCP Server`
- Click on Next
- Under Scopes, add the following scopes and click Add Scope for each

| Scope (Permission) | Display Name |
|--------------------|--------------|
| mcp:search_flights | Search Flights |
| mcp:get_locations | Get Locations |
| mcp:create_bookings | Create Bookings |
| mcp:deal-alert-consents:read | Read Deal Alert Consents |
| mcp:deal-alert-consents:write | Write Deal Alert Consents |

- Click on Create

The Identifier becomes the `aud` claim of every token issued for this server, so
it is the value that must go in the MCP server's `ASGARDEO_AUDIENCE` and in the
agents' `AGENT_RESOURCE` / `OBO_RESOURCE`. If those name a resource Asgardeo does
not recognise for the application, the `resource` parameter is silently ignored —
the token is issued with the wrong audience and the `mcp:*` scopes are dropped,
which the MCP server reports as `invalid_token` rather than `insufficient_scope`.

Documentation: https://wso2.com/asgardeo/docs/guides/agentic-ai/mcp/mcp-server-authorization/
